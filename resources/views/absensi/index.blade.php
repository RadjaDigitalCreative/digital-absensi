...
<div class="modal fade" id="staticBackdrop-{{$p->id}}{{$abs->id}}" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
			...
			<div class="col-md-12">
				<div class="card">
					<div class="card-header">
						...
						<button class="btn btn-primary">Absen 15 menit sebelum jam, {{$abs->jam_masuk}}</button>
						@endif
						@endforeach
						<br><br>
						<form action="{{ route('absensi.filter')}}" method="post">
							...
							<div class="row">
								<div class="col-md-2">
									<select data-placeholder="Pilih Bulan" name="filter" class="standardSelect" tabindex="1">
										<option value=""></option>
								...
								<div class="col-md-2">
									<button type="submit" class="btn btn-outline-primary btn-sm remove"><i class="fa fa-search"></i>&nbsp;Filter</button>
								</div>
							</div>
						</form>
					</div>
								...
								<tbody>
									...
									<tr>
										...
										<td align="center">
											...
											<button type="button" class="btn btn-success btn-sm">Masuk</button>
											@elseif($k->date == $abs->date && $k->user_id == $p->id && $k->status == 2)
											<button type="button" class="btn btn-success btn-sm">Masuk-Keluar</button>
											@elseif($k->date == $abs->date && $k->user_id == $p->id && $k->cuti == 1)
											<button type="button" class="btn btn-warning btn-sm">Cuti</button>
											@endif
											@endforeach
											<button type="button" data-toggle="modal" data-target="#staticBackdrop-{{$p->id}}{{$abs->id}}" class="btn btn-primary btn-sm">Cek</button>
										</td>
										@endif
										@endforeach
									</tr>
							...
							<table id="table2" class="table table-striped table-bordered " cellspacing="0" width="100%">
								...
								<tbody>
									...
									<tr>
										...
										@if($abs->bulan == now()->month)
										<td align="center" >
											<form action="{{ route('absensi.lembur.status', $abs->id) }}" method="post">
												@csrf
												<input type="hidden" name="date" value="{{ $abs->date }}">
												<input type="hidden" name="bulan" value="{{ $abs->bulan }}">
												<input type="hidden" name="user_id" value="{{ $p->id }}">
