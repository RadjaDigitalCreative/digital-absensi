			...
			<form action="{{ route('users.pay')}}" method="post" enctype="multipart/form-data">
				...
				<div class="modal-body">
					...
					<div class="row">
						...
						<div class="col-md-5">
							<div class="form-group">
								<input type="text" name="tgl_bayar" value="1140014145562" class="form-control" readonly="">
							</div>
						</div>
					</div>
					<div class="row">
						...
						<div class="col-md-5">
							<div class="form-group">
								...
								<select name="perusahaan_id" class="form-control">
									...
									<option value="{{$p->id}}">{{$p->nama_perusahaan}} - {{$p->lokasi}}</option>
									@endforeach
								</select>
							</div>
						</div>
						...
						<table id="ami" class="table table-striped table-bordered" cellspacing="0" width="100%">
							...
							<tbody>
								...
								<tr>
									...
									<td><button type="button" data-toggle="modal" data-target="#staticBackdrop2-{{$row->id}}" class="btn btn-primary"><i class="now-ui-icons business_money-coins"></i> Belum Ditentukan</button></td>
									@else
									<td><button type="button" data-toggle="modal" data-target="#staticBackdrop2-{{$row->id}}" class="btn btn-warning"><i class="now-ui-icons business_money-coins"></i> {{$row->nama_perusahaan}}</button></td>
									@endif
									...
									<td class="text-center">
										...
										@csrf
										@method('DELETE')
										<button type="submit" onclick="deleteRow( {{ $row->id }} )" class="btn btn-round btn-danger btn-icon  remove"><i class="menu-icon fa fa-trash"></i></button>
										...
										@csrf
										@method('DELETE')
										<button type="button" data-toggle="modal" data-target="#staticBackdrop-{{$row->id}}" class="btn btn-danger"><i class="now-ui-icons business_money-coins"></i> Bayar</button>
										<button type="submit" onclick="deleteRow( {{ $row->id }} )" class="btn btn-round btn-danger btn-icon  remove"><i class="menu-icon fa fa-trash"></i></button>
